...
class ShelveryEngine:
    ...
    def write_metadata(self):
        bucket = self._get_metadata_bucket()
        for backup in self.get_existing_backups(RuntimeConfig.get_tag_prefix()):
            self._write_backup_metadata(backup, bucket)
    
    def _get_metadata_bucket(self, aws_account_id=None):
        account_id = boto3.client('sts').get_caller_identity()['Account']
        if aws_account_id is not None:
            account_id = aws_account_id
        
        bucket_name = f"metadata.shelvery.{account_id}.base2tools"
    ...
    def _write_backup_metadata(self, backup, bucket):
        s3key = f"shelvery_meta/{self.get_engine_type()}/{backup.name}.yaml"
        bucket.put_object(
